...
/**
 * bfs 는 최단 거리 알고리즘
 * 최단 거리 알고리즘의 핵심 dist[] 배열 <-- 주로 시간을 의미
...
public class Main {
    ...
    static int N, M;
    static String[] a;
    static int[][] dist_water, dist_hedgehog; // bfs 를 2번 진행
    static boolean[][] visit; // bfs or dfs 의 필수 배열
    ...
    /**
     * bfs_water 는 multiSourceBfs 이다.
     * 이중 for 문을 이용하여
     * - visit 배열의 초기화와
     * - dist 배열의 초기화
     * - queue 에 시작점 초기화 를 해준다.
     *
     * --> 이렇게 해도 시간 복잡도는 RC 로 전체에 영향을 미치지 않는다. 왜냐면 어차피 bfs 도는 데 시간 복잡도가 RC 이기 때문이다.
     * */
    ...
    /**
     * 여기도 초기화 하는데 RC 가 걸리지만 어차피 bfs 도는데 RC 이기에 상관업다.
     * */
    static void bfs_hedgehog() {
        ...
        while (!Q.isEmpty()) {
            ...
            for (int k = 0; k < 4; k++) {
                 ...
                 * || 의 반대는 && 이다.
                 *
                 * dist_hedgehog[x][y] 이다. dist_hedgehog[nx][ny] 가 아니다.
                 *          - 왜냐면 dist_hedgehog[nx][ny] 는 아직 안정해졌고
                 * */
